...
class KnowledgeDocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return KnowledgeDocument.objects.filter(user=self.request.user).order_by(
            "-uploaded_at"
        )
        ...
        if files:
            ...
            for file_obj in files:
                ...
                try:
                    ...
                    logger.error(
                        f"Erro ao enfileirar Celery task para o doc {doc.id}: {e}. Processando de forma síncrona..."
                    )
                    try:
                        process_document_task(doc.id)
                    except Exception as sync_e:
                        logger.error(
                            f"Erro no processamento síncrono do doc {doc.id}: {sync_e}"
                        )
    ...
    def download(self, request, pk=None):
        ...
        response = FileResponse(
            doc.file.open(), content_type="application/octet-stream"
        )
