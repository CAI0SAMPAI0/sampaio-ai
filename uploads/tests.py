...
class UploadsTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="testuser@sampaio.ai", password="testpassword123"
        )
    ...
    def test_single_upload_and_processing(self):
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=True) as temp_file:
            temp_file.write(
                b"Introducao ao Python. Python e uma linguagem de programacao de alto nivel."
            )
            temp_file.seek(0)
            ...
            response = self.client.post(
                url, {"file": temp_file, "tags": "python,intro"}, format="multipart"
            )
    ...
    def test_multiple_uploads_processing(self):
        with (
            ...
            url = reverse("knowledge-document-list")
            response = self.client.post(
                url, {"files": [file1, file2], "tags": "multi"}, format="multipart"
            )
