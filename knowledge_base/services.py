    ...
    try:
        if ext == "pdf":
            ...
            try:
                ...
                text_by_page.append(
                    (1, f"[Erro ou DOCX sem texto legível: {filename}]")
                )
        elif ext == "epub":
            try:
                import zipfile
                ...
                with zipfile.ZipFile(f_obj) as epub:
                    ...
                    for file_name in content_files:
                        try:
                            content = epub.read(file_name).decode(
                                "utf-8", errors="ignore"
                            )
                    ...
                    if full_text:
                        ...
                        text_by_page.append(
                            (1, f"[EPUB sem texto legível: {filename}]")
                        )
            except Exception as e:
            ...
            text_by_page.append(
                (1, f"[Conteúdo indexado do arquivo {ext.upper()}: {filename}]")
            )
    finally:
        if should_close:
            f_obj.close()
