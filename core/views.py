...
def _dispatch_sync_agent(session_id, user_id, messages_data):
    ...
    with _local_tasks_lock:
        _local_tasks[task_id] = {"state": "PENDING", "result": None}
    t = threading.Thread(
...
def task_status(request, task_id):
    ...
    try:
        ...
        if result.state == "PENDING":
            return JsonResponse(
                {"state": "PENDING", "status": "Aguardando processamento..."}
            )
            ...
            return JsonResponse({"state": result.state, "status": "Processando..."})
    except Exception as e:
        return JsonResponse(
            {"state": "FAILURE", "error": f"Erro ao consultar tarefa: {str(e)}"},
            status=500,
        )
...
def chat_page(request):
    ...
    return render(
        request,
        "chat.html",
        {"sessions": sessions, "active_session": active_session, "messages": messages},
    )
...
def new_chat(request):
    if request.method == "POST":
        ...
        session = ChatSession.objects.create(
            user=request.user, title=f"Conversa #{num_sessions}"
        )
    ...
    try:
        ...
        if request.method == "POST":
            ...
            user_msg = ChatMessage.objects.create(
                session=session, role="user", content=full_content
            )
...
def library_page(request):
    documents = KnowledgeDocument.objects.filter(user=request.user).order_by(
        "-uploaded_at"
    )
...
def flashcards_page(request):
    ...
    return render(
        request, "flashcards.html", {"flashcards": flashcards, "documents": documents}
    )
...
def quizzes_page(request):
    ...
    return render(request, "quizzes.html", {"quizzes": quizzes, "documents": documents})


@login_required(login_url="login_page")
def study_plans_page(request):
    plans = StudyPlan.objects.filter(user=request.user).order_by("-created_at")
    return render(request, "study_plans.html", {"plans": plans})
...
def challenges_page(request):
    ...
    difficulty = level_to_difficulty.get(user_level, "iniciante")
    ...
    challenge = (
        DailyChallenge.objects.filter(date=today, difficulty=difficulty)
        .exclude(id__in=passed_challenge_ids)
        .first()
    )

    if not challenge:
        challenge = (
            DailyChallenge.objects.filter(date=today)
            .exclude(id__in=passed_challenge_ids)
            .first()
        )
    ...
    if challenge:
        submission = (
            ChallengeSubmission.objects.filter(user=request.user, challenge=challenge)
            .order_by("-created_at")
            .first()
        )
...
def run_challenge_code(request):
    ...
    try:
        ...
        result = subprocess.run(
            [python_exec, temp_path], capture_output=True, text=True, timeout=5
        )
        ...
        return JsonResponse(
            {"passed": False, "stdout": "", "stderr": timeout_msg, "exit_code": -1}
        )
    except Exception as e:
        return JsonResponse(
            {
...
def submit_challenge(request):
    ...
    try:
        ...
        task = submit_challenge_task.delay(challenge_id, request.user.id, code)
...
def public_chat_share_view(request, session_id, token):
    ...
    messages = session.messages.all().order_by("created_at")
    return render(
        request, "shared_chat.html", {"session": session, "messages": messages}
    )
...
def run_terminal_command(request):
    ...
    sandbox_dir = os.path.join(
        settings.MEDIA_ROOT, "playground", f"user_{request.user.id}"
    )
...
def analyze_user_level(request):
    ...
    try:
        ...
        try:
            ...
            task = analyze_user_level_task.delay(request.user.id, log)
            return JsonResponse(
                {"success": True, "task_id": task.id, "status": "processing"}
            )
        except Exception:
            pass
...
def edit_chat_message(request, message_id):
    ...
    try:
        ...
        sync_task_id = _dispatch_sync_agent(session.id, request.user.id, messages_data)
        return JsonResponse(
            {
                "success": True,
                "task_id": sync_task_id,
...
def chat_sessions_fragment(request):
    ...
    sessions = ChatSession.objects.filter(user=request.user).order_by("-created_at")
    return render(request, "fragments/chat_sessions.html", {"sessions": sessions})


@login_required(login_url="login_page")
def document_status_fragment(request, document_id):
    """Retorna o status de processamento de um documento."""
    doc = get_object_or_404(KnowledgeDocument, id=document_id, user=request.user)
    return render(request, "fragments/document_status.html", {"doc": doc})
...
def dashboard_stats_fragment(request):
    ...
    return render(
        request,
        "fragments/dashboard_stats.html",
        {
            "arquivos_enviados": docs_stats["total"],
            "documentos_processados": docs_stats["processed"],
            "chats_realizados": ChatSession.objects.filter(user=user).count(),
