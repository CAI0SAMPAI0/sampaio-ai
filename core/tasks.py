...
def invoke_chat_agent_task(self, session_id, user_id, messages_data):
    ...
    try:
        ...
        for msg_data in messages_data:
            if msg_data["role"] == "user":
                lc_messages.append(HumanMessage(content=msg_data["content"]))
            else:
                lc_messages.append(AIMessage(content=msg_data["content"]))
        ...
        assistant_content = (
            f"Desculpe, ocorreu um erro no processamento do agente: {str(e)}"
        )
    ...
    return {"assistant_content": assistant_content, "assistant_message_id": ai_msg.id}
...
def verify_flashcard_answer_task(self, flashcard_id, user_answer):
    ...
    if groq_key and groq_key != "gsk_placeholder_for_development" and groq_key != "":
        try:
            ...
            response_content = invoke_groq_with_fallback(
                [HumanMessage(content=prompt)], temperature=0.2
            )
            match = re.search(r"\{.*\}", response_content, re.DOTALL)
            if match:
...
def generate_flashcards_task(self, document_id, user_id):
    ...
    try:
        doc = KnowledgeDocument.objects.get(id=document_id, user_id=user_id)
    ...
    return {
        ...
        "flashcards": [{"front": f.front, "back": f.back} for f in flashcard_objs],
    }


@shared_task(bind=True)
def generate_quiz_task(
    self, document_id, user_id, theme=None, num_questions=3, difficulty="Médio"
):
    """
    ...
    quiz = Quiz.objects.create(user_id=user_id, document=doc, title=title)
    ...
    if not questions:
        questions = [
            QuizQuestion(
                quiz=quiz,
                question_text=(f"Melhores práticas em {doc.name}?"),
                options=[
                    "Testes e PEP8",
                    "Sem testes",
                    "Arquivos gigantes",
...
def generate_study_plan_task(
    self, user_id, objective, technology, available_hours, duration_weeks
):
    """
            ...
            semanas.append(
                {
                    ...
                    "tarefas": [f"Ler sobre {technology}", "Praticar com projetos"],
