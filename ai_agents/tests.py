...
class AgentTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="testagent@sampaio.ai", password="password123"
        )
        ...
        self.doc.file.save(
            "react_notes.txt",
            ContentFile(
                b"React utiliza virtual DOM para otimizar a renderizacao da interface."
            ),
        )
        self.doc.save()
    ...
    def test_langgraph_agent_rag_execution(self):
        messages = [
            HumanMessage(
                content="React utiliza virtual DOM para otimizar a renderizacao da interface."
            )
        ]
        state = {
            "messages": messages,
